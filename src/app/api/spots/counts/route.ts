  ...
  try {
    ...
    const baseCount = () =>
      supabase
        ...
        .eq("spot_id", spot_id) // ✅ slug
        .eq("country", "DK")
        .eq("visibility", "public_aggregate");

    const totalQ = await baseCount();
    if (totalQ.error) return NextResponse.json({ ok: false, error: totalQ.error.message }, { status: 500 });

    const qtrQ = await baseCount().gte("created_at", quarterStartISO());
    if (qtrQ.error) return NextResponse.json({ ok: false, error: qtrQ.error.message }, { status: 500 });

    const last30Q = await baseCount().gte("created_at", daysAgoISO(30));
    if (last30Q.error) return NextResponse.json({ ok: false, error: last30Q.error.message }, { status: 500 });
    ...
    if (firstQ.error) return NextResponse.json({ ok: false, error: firstQ.error.message }, { status: 500 });
    ...
    if (lastQ.error) return NextResponse.json({ ok: false, error: lastQ.error.message }, { status: 500 });
