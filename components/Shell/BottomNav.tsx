...
function Icon({ kind }: { kind: string }) {
  const common = { className: styles.icon, viewBox: "0 0 24 24", "aria-hidden": true } as const;
  if (kind === "season")
    return (
      <svg {...common}>
        <path d="M12 3c3.9 0 7 3.1 7 7 0 4.4-3.6 9-7 11-3.4-2-7-6.6-7-11 0-3.9 3.1-7 7-7Z" fill="none" stroke="currentColor" strokeWidth="1.8"/>
      </svg>
    );
  if (kind === "log")
    return (
  ...
  if (kind === "map")
    return (
      <svg {...common}>
        <path d="M9 18 3 20V6l6-2 6 2 6-2v14l-6 2-6-2Z" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round"/>
      </svg>
    );
  if (kind === "me")
    return (
    ...
    <nav className={styles.nav}>
      <div className={styles.inner}>
        {items.map((it) => {
          ...
          return (
            <Link key={it.key} href={href} className={active ? styles.itemActive : styles.item}>
              <Icon kind={it.icon} />
